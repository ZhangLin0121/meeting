...
#### 设备配置点选修复 (Latest)
**问题**: 添加会议室功能中，设备配置选项点击无效
**根本原因**: 
- `toggleEquipment`函数只更新了`roomForm.equipment`数组
- 未同步更新`equipmentSelection`对象状态
- 导致UI显示的选中状态与实际数据不一致

**解决方案**:
1. 修复`toggleEquipment`函数，同时更新两个状态
2. 添加调试日志追踪设备选择变更
3. 确保UI正确反映设备选中状态

**修改文件**:
- `frontend/pages/admin/admin.js` - 修复设备选择逻辑
